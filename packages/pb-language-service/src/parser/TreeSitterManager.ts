...
export class TreeSitterManager {
  ...
  constructor() {
    ...
    this.parser.setLanguage(PowerBuilder);
  ...
  updateWithChanges(
    ...
    for (const change of changes) {
      if ('range' in change && change.range) {
        // Mudança incremental
        const startIndex = this.positionToIndex(text, change.range.start.line, change.range.start.character);
        const oldEndIndex = this.positionToIndex(text, change.range.end.line, change.range.end.character);
